        ...
        <Card className="w-full lg:w-[460px] bg-white rounded-[12px] border border-gray-200 overflow-hidden shadow-sm">
          <CardContent className="p-0">
            <img
              ...
              src={featuredArticle?.imageUrl || getDynamicPlaceholder(featuredArticle?.title || '', featuredArticle?.sourceName || '')}
              onError={(e) => {
                (e.target as HTMLImageElement).src = getDynamicPlaceholder(featuredArticle?.title || '', featuredArticle?.sourceName || '');
              }}
            />
          ...
          {sideArticles.map((article, index) => (
            ...
            >
              <img
                ...
                src={article?.imageUrl || getDynamicPlaceholder(article?.title || '', article?.sourceName || '')}
                onError={(e) => {
                  (e.target as HTMLImageElement).src = getDynamicPlaceholder(article?.title || '', article?.sourceName || '');
                }}
              />
