...
export const AdminPage: React.FC<AdminPageProps> = () => {
  ...
  const createProduct = async (productData: any) => {
    try {
      ...
      // Note: Products will refresh automatically on page reload
    } catch (error) {
        ...
        <Tabs defaultValue="content" value={currentTab} onValueChange={setCurrentTab} className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-[#e1e6eb] p-4">
            <TabsList className="w-full overflow-x-auto inline-flex gap-2 bg-[#007bff08] rounded-lg p-2 sm:grid sm:grid-cols-3 lg:grid-cols-6 snap-x snap-mandatory scroll-smooth [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [-webkit-overflow-scrolling:touch] overscroll-x-contain touch-pan-x">
              <TabsTrigger 
                ...
                value="content"
                className="[font-family:'Nunito',Helvetica] font-semibold text-sm snap-start flex-shrink-0 min-w-[120px] h-10 px-3 py-2 whitespace-nowrap text-[#333333] data-[state=active]:bg-[#28a745] data-[state=active]:text-white rounded-md transition-all duration-200"
              >
                Site Content
              </TabsTrigger>
              <TabsTrigger 
          ...
          <TabsContent value="categories">
            <Card className="bg-white border-[#e1e6eb] shadow-sm rounded-xl">
              ...
              <CardContent className="p-6">
                <CategoryManager categories={categories} onUpdate={loadAdminData} />
              </CardContent>
            </Card>
          </TabsContent>
...
const ProductManager: React.FC<{
  ...
  onImageUpload: (file: File) => Promise<string | null>;
}> = ({ products, categories, onCreate, onDelete, onImageUpload }) => {
          ...
          {categories.map((category) => (
            <div key={category.id} className="flex justify-between items-center p-3 bg-gray-50 rounded">
              <div>
                ...
                <span className="text-sm text-gray-500 ml-2">Order: {category.displayOrder}</span>
              </div>
            </div>
          ))}
        </div>
